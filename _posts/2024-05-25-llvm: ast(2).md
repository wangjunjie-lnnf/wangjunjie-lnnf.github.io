            ...
            PastIdentifier:
                if (Tok.is(tok::l_paren)) {
                    ...
                    ParseScope PrototypeScope(this,
                                Scope::FunctionPrototypeScope|Scope::DeclScope|Scope::FunctionDeclarationScope, ...);
...
以上可以看出`AST`的节点是由`Stmt`构成的，`Stmt`有两个特列: `DeclStmt`和`Expr`, 
分别用于表示声明语句和表达式。
...
public:
    ...
    // 采用header+body的模式以节省内存
    // 通过header里的各种bit来解释body
...
class TrailingObjects : private trailing_objects_internal::TrailingObjectsImpl<...> {
    ...
    const T *getTrailingObjects() const {
        return this->getTrailingObjectsImpl(static_cast<const BaseTy *>(this), ...);
    }

    static const BaseTy * getTrailingObjectsImpl(const BaseTy *Obj, ...) {
        return Obj;
    }
}
...
class IfStmt final
    ...
    static IfStmt *Create(const ASTContext &Ctx, IfStatementKind Kind, 
                          Stmt *Init, VarDecl *Var,
                          Expr *Cond, Stmt *Then, Stmt *Else = nullptr);
    {
        ...
        // 指定实例的内存
        return new (Mem) IfStmt(Ctx, Kind, Init, Var, Cond, Then, EL, Else);
        {
            // 设置header
            IfStmtBits.HasElse = Else != nullptr;
            IfStmtBits.HasVar = Var != nullptr;
            IfStmtBits.HasInit = Init != nullptr;
...
// 创建函数原型作用域, 源码处理到`int fun(`
ParseScope PrototypeScope(this, Scope::FunctionPrototypeScope|Scope::DeclScope|Scope::FunctionDeclarationScope, ...);

// 解析函数原型开始, 源码处理到`int fun(`
void Sema::ActOnStartFunctionDeclarationDeclarator(Declarator &Declarator, ...) {
    // 处理c++模板参数
}

// 开始解析函数参数, 源码处理到`int fun(int i`
Decl *Sema::ActOnParamDeclarator(Scope *S, Declarator &D, ...) {
    // 在当前作用域及其父作用域查找当前声明的变量名
    IdentifierInfo *II = D.getIdentifier();
    LookupResult R(*this, II, ...);
    LookupName(R, S);
    ...
    // 创建参数声明语句
    ParmVarDecl *New = CheckParameter(...);
    {
        New = ParmVarDecl::Create(...);
    }
...
ParseScope BodyScope(this, Scope::FnScope | Scope::DeclScope | Scope::CompoundStmtScope);

// 解析函数body开始, 源码处理到`int fun(int i) {`
Decl * Sema::ActOnStartOfFunctionDef(Scope *FnBodyScope, Declarator &D, ...) {
    ...
    {
        NamedDecl *New = ActOnFunctionDeclarator(S, D, DC, ...);
        {
    ...
    {
        FunctionDecl *FD = cast<FunctionDecl>(DP);
        FD->setWillHaveBody();
...
// 函数body解析完毕
StmtResult Sema::ActOnCompoundStmt(...) {
