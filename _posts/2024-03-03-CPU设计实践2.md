...
#pragma HLS ARRAY_PARTITION variable=f_state         dim=1 complete
  ...
  from_f_to_d_t f_to_d;
  from_f_to_d_t d_from_f;
  // d_state_is_full[hart]会导致hart在取指阶段不能被调度
...
每个阶段的处理流程：  
1. 选择可调度的hart
2. 保存有效的输入数据到state待被调度时处理
3. 判断是否有可调度的hart
4. 处理可调度的hart: 保存处理结果到state，更新输出数据
5. 设置输出数据是否有效
...
#pragma HLS INLINE
  ...
  d_state[hart].relative_pc = (d_i.is_jal) ?
      (code_address_t)(d_state[hart].fetch_pc + (code_address_t)(d_i.imm >> 1)) :
...
void decode(
  ...
  select_hart(d_state_is_full, i_state_is_full, &is_selected, &selected_hart);
  ...
  is_decoding   = is_selected ||
                  (d_from_f.is_valid && !i_state_is_full[d_from_f.hart]);
  decoding_hart = (is_selected) ? selected_hart : d_from_f.hart;
...
static void save_input_from_d(
  ...
  // 判断ReadAfterWrite依赖
  is_locked_1 = i_from_d.d_i.is_rs1_reg   &&
                is_reg_computed[hart][i_from_d.d_i.rs1];
  is_locked_2 = i_from_d.d_i.is_rs2_reg   &&
                is_reg_computed[hart][i_from_d.d_i.rs2];
  i_state[hart].wait_12     = is_locked_1 || is_locked_2;
}
...
static void select_hart(
  ...
  is_locked_1      [0] = i_state        [0].d_i.is_rs1_reg   &&
                         is_reg_computed[0][i_state[0].d_i.rs1];
  is_locked_2      [0] = i_state        [0].d_i.is_rs2_reg   &&
                         is_reg_computed[0][i_state[0].d_i.rs2];
  wait_12          [0] = is_locked_1[0] || is_locked_2[0];

  // 是否可调度
  c[0] = (i_state_is_full[0] && !e_state_is_full[0] && !wait_12[0]);

  is_locked_1      [1] = i_state        [1].d_i.is_rs1_reg &&
                         is_reg_computed[1][i_state[1].d_i.rs1];
  is_locked_2      [1] = i_state        [1].d_i.is_rs2_reg &&
                         is_reg_computed[1][i_state[1].d_i.rs2];
  wait_12          [1] = is_locked_1[1] || is_locked_2[1];

  c[1] = (i_state_is_full[1] && !e_state_is_full[1] && !wait_12[1]);
...
void issue(
  ...
  is_issuing   = is_selected ||
                (i_from_d.is_valid && !e_state_is_full[i_from_d.hart] &&
                !i_state[i_from_d.hart].wait_12);
  issuing_hart = (is_selected) ? selected_hart : i_from_d.hart;

  // 是否在回写阶段更新寄存器
  *is_lock     = is_issuing && !i_state[issuing_hart].d_i.has_no_dest;
...
static void stage_job(
  ...
  // 跳转目标
  e_state[hart].target_pc = ((e_state[hart].d_i.is_branch && bcond) ||
                              e_state[hart].d_i.is_jalr) ? computed_pc 
                                                         : e_state[hart].relative_pc;
  // 是否跳转
  e_state[hart].is_target = (e_state[hart].d_i.is_branch ||
                              (e_state[hart].d_i.is_jalr &&
                                (!e_state[hart].d_i.is_ret || (computed_pc != 0))));
}
static void set_output_to_f(
  hart_num_t     hart,
  e_state_t     *e_state,
...
static void set_output_to_m(
  ...
  e_to_m->value       =
    ...
    (e_state[hart].d_i.is_store) ? e_state[hart].rv2 : result1;
}
...
void execute(
  ...
  if (is_executing) {
    ...
    compute  (executing_hart, e_state, &bcond, &result1, &result2, &computed_pc);
    // 存储执行的结果
  ...
  // 分支且跳转
  e_to_f->is_valid = is_executing && e_state[executing_hart].is_target;
...
static void save_input_from_e(
  ...
  m_state[hart].accessed_h  = (m_from_e.address>>(LOG_HART_DATA_RAM_SIZE+2)) + hart;
...
static void stage_job(
  ...
  if (is_load)
    *value = mem_load(hart, data_ram, address, func3);
    ...
    mem_store(hart, data_ram, address, *value, (ap_uint<2>)func3);
}
...
void mem_access(
  ...
  select_hart(m_state_is_full, w_state_is_full, &is_selected, &selected_hart);
  ...
  is_accessing   = is_selected ||
                  (m_from_e.is_valid && !w_state_is_full[m_from_e.hart]);
  accessing_hart = (is_selected) ? selected_hart : m_from_e.hart;
...
void write_back(
  ...
  // 选择可调度的hart
  select_hart(w_state_is_full, &is_selected, &selected_hart);
  ...
  // 是否有可调度的hart
  is_writing   = is_selected || w_from_m.is_valid;
  writing_hart = (is_selected) ? selected_hart : w_from_m.hart;

  // 标识寄存器已更新完成
  *is_unlock = is_writing && !w_state[writing_hart].has_no_dest;
  // 更新完成的寄存器
